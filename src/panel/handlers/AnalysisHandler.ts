...
export class AnalysisHandler {
  ...
  async analyzeLocal() {
    ...
    const fileSummaries = await summarizeFiles(
      this.state.provider, graph.nodes, wsInfo.files,
      (done, total) => this.post({ type: "progress", payload: { step: 4, message: `Summarizing files ${done}/${total}...` } })
    );
    this.state.currentFileSummaries = fileSummaries;
    this.post({ type: "fileSummariesReady", payload: fileSummaries });

    // ── Step 5: Flow analysis (static) + AI flow detection ──
    this.post({ type: "progress", payload: { step: 5, message: "Analyzing data flows..." } });
    const flowContext = analyzeFlows(wsInfo.files);
    this.state.currentFlowContext = flowContext;

    const flowMap = await autoDetectFlows(
      this.state.provider, wsInfo, summary, fileSummaries, flowContext
    );
